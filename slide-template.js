 ...
 */
function generateSlideHTML(slideData, duration) { // Extract data from the slide
const {
    SLIDE_TEXT = "Title",
    SLIDE_BULLETS = [],
...
const titleDelay = 0.3; // Title appears at 0.3s const bulletDelay = 0.8; // First bullet at 0.8s const bulletGap = 0.4; // 0.4s between bullets
  // Generate bullet HTML with staggered animations
const bulletsHTML = SLIDE_BULLETS.map((bullet, index) => { const delay = bulletDelay + (index * bulletGap);
return `
...
let chartHTML = ''; if (hasChart) {
const maxValue = Math.max(...Object.values(VISUAL_DATA)); const chartBars = Object.entries(VISUAL_DATA).map(([label,
value], index) => {
const height = (value / maxValue) * 100; const delay = 1.5 + (index * 0.3); return `
